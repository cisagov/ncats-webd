...
def severity_calculation(db, severity):
    ...
    customer_list = [
        s["_id"]
        for s in db.RequestDoc.collection.find(
            {"scan_types": SCAN_TYPE.CYHY}, {"_id": 1}
        )
    ]
    ...
    pipeline = [
        {
            "$match": {
                "owner": {"$in": customer_list},
                "tix_msec_open": {"$exists": True},
            }
        },
        {
            "$project": {
    ...
    sorted_by_second1 = sorted(
        sev_customer_bundle, key=lambda tup: tup[1], reverse=True
    )
    sorted_by_second2 = sorted(
        open_customer_bundle, key=lambda tup: tup[1], reverse=True
    )
    sorted_by_second3 = sorted(
        close_customer_bundle, key=lambda tup: tup[1], reverse=True
    )
...
def get_ranking_lists(db):
    ...
    avg_max_time_mitigate_critical_list = sorted(
        critical_list[2], key=lambda tup: tup[0]
    )
    ...
    avg_relative_score = []
    for a, b, c, d in zip(
        avg_relative_critical_list,
        avg_relative_high_list,
        avg_relative_medium_list,
        avg_relative_low_list,
    ):
    ...
    for i in sorted_by_second1:
        average_vulnerability_ranking.append(
            (
                i[0],
                db.RequestDoc.find_one({"_id": i[0]})["agency"]["name"],
                round(i[1], 2),
            )
        )
    ...
    for a, b, c, d in zip(
        avg_max_vuln_alive_critical_list,
        avg_max_vuln_alive_high_list,
        avg_max_vuln_alive_medium_list,
        avg_max_vuln_alive_low_list,
    ):
        avg_max_vuln_alive_score.append((a[0], a[3] + b[3] + c[3] + d[3]))

    sorted_by_second2 = sorted(
        avg_max_vuln_alive_score, key=lambda tup: tup[1], reverse=True
    )
    for i in sorted_by_second2:
        average_max_vulnerability_alive_ranking.append(
            (
                i[0],
                db.RequestDoc.find_one({"_id": i[0]})["agency"]["name"],
                round(i[1], 2),
    ...
    for a, b, c, d in zip(
        avg_max_time_mitigate_critical_list,
        avg_max_time_mitigate_high_list,
        avg_max_time_mitigate_medium_list,
        avg_max_time_mitigate_low_list,
    ):
        avg_max_time_mitigate_score.append((a[0], a[3] + b[3] + c[3] + d[3]))

    sorted_by_second3 = sorted(
        avg_max_time_mitigate_score, key=lambda tup: tup[1], reverse=True
    )
    for i in sorted_by_second3:
        average_max_time_to_mitigate.append(
            (
                i[0],
                db.RequestDoc.find_one({"_id": i[0]})["agency"]["name"],
                round(i[1], 2),
    ...
    for a, b, c in zip(
        avg_relative_score, avg_max_vuln_alive_score, avg_max_time_mitigate_score
    ):
    ...
    sorted_by_second4 = sorted(
        total_relative_score, key=lambda tup: tup[1], reverse=True
    )
