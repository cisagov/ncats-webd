...
class HiringDashboardQueries:
    ...
    def get_current_billets(db):
        ...
        billets = db.new_hire.find(
            {"active": True},
            {
                "position_number": 1,
                "insert_date": 1,
                "date_stage_entered": 1,
                "current_stage": 1,
                "eod": 1,
                "billet_started": 1,
                "sub_component": 1,
            },
        ).sort([("billet_started", 1), ("stage.current", -1)])
        return list(billets)
...
class DashboardQueries:
    ...
    def get_overall_metrics(db, stakeholders):
        ...
        results["vulnerable_hosts"] = len(
            db.tickets.find({"open": True, "false_positive": False}).distinct("ip_int")
        )
                ...
                [
                    ...
                    },
                    {
                        "$group": {
                            ...
                            "low_tix_open": {
                                "$sum": {
                                    "$cond": [{"$eq": ["$details.severity", 1]}, 1, 0]
                                }
                            },
                            "medium_tix_open": {
                                "$sum": {
                                    "$cond": [{"$eq": ["$details.severity", 2]}, 1, 0]
                                }
                            },
                            "high_tix_open": {
                                "$sum": {
                                    "$cond": [{"$eq": ["$details.severity", 3]}, 1, 0]
                                }
                            },
                            "critical_tix_open": {
                                "$sum": {
                                    "$cond": [{"$eq": ["$details.severity", 4]}, 1, 0]
                                }
                            },
    ...
    def get_tally_details(db):
        results = dict()
        stage_status_counts = list(
            db.tallies.aggregate(
                [
                    {
                        "$group": {
                            "_id": "null",
    ...
    def get_election_metrics(db):
        ...
        results["hosts"] = db.hosts.find(
            {"state.up": True, "owner": {"$in": election_orgs}}
        ).count()
        results["vulnerable_hosts"] = len(
            db.tickets.find(
                {
