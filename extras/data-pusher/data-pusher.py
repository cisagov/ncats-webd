...
def push_data(source_file_list, destination_dir):
    for filename in source_file_list:
        ...
        if len(paths.failed) == 0:
            logger.info('{!s} was pushed successfully to {!s}'.format(filename, env.host_string))
        else:
            logger.error('Error pushing {!s} to host {!s}'.format(filename, env.host_string))
...
def priority_counts_pl():
    return  [
            ...
            {'$group': {'_id': {'status':'$status', 'stage':'$stage', 'priority':'$priority'},
                        'total':{'$sum':1},
                        }
            },
            {'$sort': {'_id.priority':1}}
...
def get_tally_details(db):
    results = dict()
    stage_status_counts = db.hosts.aggregate([{'$group':{'_id':{'stage':'$stage', 'status':'$status'}, 
                                                         'count':{'$sum':1}}}])     
    for (queue_name, stage, status) in [('ns1_waiting', STAGE.NETSCAN1, [STATUS.WAITING, STATUS.READY]),
...
def generate_push_data_files(db):
    ...
    for (output_file, json_data) in [(TICKET_COUNT_FILENAME, ticket_severity_counts),
        ...
        with open(output_file, 'wb') as out:
            out.write(json.dumps(json_data, sort_keys=True, default=util.custom_json_handler))
