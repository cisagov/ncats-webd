...
def main():
    ...
    if args['--debug']: arg_list.append("debug=" + str(args['--debug']))
    if args['--local']: arg_list.append("local=" + str(args['--local']))
    if args['--secret-key']: arg_list.append("secret_key='" + args['--secret-key'] + "'")
    if args['--config']: arg_list.append("config_filename='" + args['--config'] + "'")
    if args['--section']: arg_list.append("section='" + args['--section'] + "'")
    if args['--new_hire_section']: arg_list.append("new_hire_section='" + args['--new_hire_section'] + "'")
    command += ",".join(arg_list) + ")\""

    if args['--debug']: print("Launching %s" % command)
