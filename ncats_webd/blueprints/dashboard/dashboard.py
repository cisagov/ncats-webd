...
class TicketFeed(object):
    ...
    def __check_database(self):
        ...
        tickets = self.__db.tickets.find(
            {"source": "nessus", "last_change": {"$gt": self.__since}},
            {
                "_id": 1,
                "owner": 1,
                "details": 1,
...
def start_background_thread():
    ...
    if ticket_feed is None:
        ticket_feed = TicketFeed(
            current_app.db, broadcast_new_tickets, current_app.logger
        )
...
def json_get_ticket_severity_counts():
    ...
    sorted_ticket_data = DashboardQueries.get_ticket_severity_counts(
        current_app.db, current_app.db.requests.find({"stakeholder": True})
    )
...
def json_get_election_ticket_severity_counts():
    ...
    sorted_ticket_data = DashboardQueries.get_ticket_severity_counts(
        current_app.db,
        current_app.db.requests.find({"_id": {"$in": election_stakeholders}}),
    )
    return json.dumps(sorted_ticket_data, default=util.custom_json_handler)
...
def ticket_query_source(source_id):
    ...
    doc = list(
        current_app.db.TicketDoc.find({"source": source, "source_id": int(source_id)})
    )
    return json.dumps(doc, default=util.custom_json_handler)
...
def latest_ticket_severity_counts_event():
    ...
    emit(
        ...
        {"data": json_get_election_ticket_severity_counts()},
    )
...
def broadcast_ticket_severity_counts():
    ...
    socketio.emit(
        ...
        {"data": ticket_severity_counts},
        namespace="/cyhy",
        room="ticket_severity_counts",
    )
...
def broadcast_election_metrics():
    ...
    socketio.emit(
        "election_metrics",
        {"data": election_metrics},
        namespace="/cyhy",
        room="election_metrics",
    )
...
def broadcast_election_ticket_severity_counts():
    ...
    socketio.emit(
        "eleciton_ticket_severity_counts",
        {"data": ticket_severity_counts},
        namespace="/cyhy",
        room="election_ticket_severity_counts",
    )
