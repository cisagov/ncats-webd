...
def get_owner_types_ci(db, as_lists=False, stakeholders_only=False):
    ...
    for agency_type in CI_CHILDREN:
        ...
        if stakeholders_only:
            for org in db.RequestDoc.find(
                {"_id": {"$in": all_agency_type_descendants}}
            ):
                if org["stakeholder"]:
...
def scanning_breakdown(db):
    ...
    ci_stakeholders_by_type = get_owner_types_ci(
        db, as_lists=True, stakeholders_only=True
    )

    stakeholders_scanned["chemical"] = len(ci_stakeholders_by_type["CI_CHEMICAL"])
    stakeholders_scanned["commercial_facilities"] = len(
        ci_stakeholders_by_type["CI_COMMERCIAL_FACILITIES"]
    )
    stakeholders_scanned["communications"] = len(
        ci_stakeholders_by_type["CI_COMMUNICATIONS"]
    )
    stakeholders_scanned["critical_manufacturing"] = len(
        ci_stakeholders_by_type["CI_CRITICAL_MANUFACTURING"]
    )
    stakeholders_scanned["dams"] = len(ci_stakeholders_by_type["CI_DAMS"])
    stakeholders_scanned["defense_industrial_base"] = len(
        ci_stakeholders_by_type["CI_DEFENSE_INDUSTRIAL_BASE"]
    )
    stakeholders_scanned["emergency_services"] = len(
        ci_stakeholders_by_type["CI_EMERGENCY_SERVICES"]
    )
    stakeholders_scanned["energy"] = len(ci_stakeholders_by_type["CI_ENERGY"])
    stakeholders_scanned["financial_services"] = len(
        ci_stakeholders_by_type["CI_FINANCIAL_SERVICES"]
    )
    stakeholders_scanned["food_and_agriculture"] = len(
        ci_stakeholders_by_type["CI_FOOD_AND_AGRICULTURE"]
    )
    stakeholders_scanned["government_facilities"] = len(
        ci_stakeholders_by_type["CI_GOVERNMENT_FACILITIES"]
    )
    stakeholders_scanned["healthcare_and_public_health"] = len(
        ci_stakeholders_by_type["CI_HEALTHCARE_AND_PUBLIC_HEALTH"]
    )
    stakeholders_scanned["information_technology"] = len(
        ci_stakeholders_by_type["CI_INFORMATION_TECHNOLOGY"]
    )
    stakeholders_scanned["nuclear_reactors_materials_and_waste"] = len(
        ci_stakeholders_by_type["CI_NUCLEAR_REACTORS_MATERIALS_AND_WASTE"]
    )
    stakeholders_scanned["transportation_systems"] = len(
        ci_stakeholders_by_type["CI_TRANSPORTATION_SYSTEMS"]
    )
    stakeholders_scanned["water_and_wastewater_systems"] = len(
        ci_stakeholders_by_type["CI_WATER_AND_WASTEWATER_SYSTEMS"]
    )
...
def ticket_counts_by_fiscal_year(db, current_fy_start):
    ...
    while True:
        ...
        tix_opened = db.TicketDoc.find(
            {
                "source": "nessus",
                "time_opened": {"$gte": current_fy_start, "$lt": next_fy_start},
                "false_positive": False,
            }
        ).count()
        # Tickets open at any point during the fiscal year
        open_tix = db.TicketDoc.find(
            {
                "source": "nessus",
                "$or": [
                    {
                    ...
                    {
                        "$and": [
                            {"time_opened": {"$lt": current_fy_start}},
                            {"open": True},
                            {"false_positive": False},
                        ]
                    },
        ...
        tix_closed = db.TicketDoc.find(
            {
                "source": "nessus",
                "time_closed": {"$gte": current_fy_start, "$lt": next_fy_start},
                "false_positive": False,
            }
        ).count()
...
def all_time_opened_and_closed_breakdown(db):
    ...
    all_time_opened_and_closed = {
        "total_opened_since_inception": "{:,d}".format(
            db.TicketDoc.find({"source": "nessus", "false_positive": False}).count()
        )
    }
    all_time_opened_and_closed[
        "total_opened_since_inception_critical"
    ] = "{:,d}".format(
        db.TicketDoc.find(
            {"source": "nessus", "false_positive": False, "details.severity": 4}
        ).count()
    )
    all_time_opened_and_closed["total_opened_since_inception_high"] = "{:,d}".format(
        db.TicketDoc.find(
            {"source": "nessus", "false_positive": False, "details.severity": 3}
        ).count()
    )
    all_time_opened_and_closed["total_opened_since_inception_medium"] = "{:,d}".format(
        db.TicketDoc.find(
            {"source": "nessus", "false_positive": False, "details.severity": 2}
        ).count()
    )
    all_time_opened_and_closed["total_opened_since_inception_low"] = "{:,d}".format(
        db.TicketDoc.find(
            {"source": "nessus", "false_positive": False, "details.severity": 1}
        ).count()
    )

    all_time_opened_and_closed["currently_open"] = "{:,d}".format(
        db.TicketDoc.find(
            {"source": "nessus", "false_positive": False, "open": True}
        ).count()
    )
    all_time_opened_and_closed["currently_open_critical"] = "{:,d}".format(
        db.TicketDoc.find(
            {
                "source": "nessus",
                "false_positive": False,
    ...
    all_time_opened_and_closed["currently_open_high"] = "{:,d}".format(
        db.TicketDoc.find(
            {
                "source": "nessus",
                "false_positive": False,
    ...
    all_time_opened_and_closed["currently_open_medium"] = "{:,d}".format(
        db.TicketDoc.find(
            {
                "source": "nessus",
                "false_positive": False,
    ...
    all_time_opened_and_closed["currently_open_low"] = "{:,d}".format(
        db.TicketDoc.find(
            {
                "source": "nessus",
                "false_positive": False,
    ...
    all_time_opened_and_closed["total_closed_since_inception"] = "{:,d}".format(
        db.TicketDoc.find(
            {"source": "nessus", "false_positive": False, "open": False}
        ).count()
    )
    all_time_opened_and_closed[
        "total_closed_since_inception_critical"
    ] = "{:,d}".format(
        db.TicketDoc.find(
            {
                "source": "nessus",
    ...
    all_time_opened_and_closed["total_closed_since_inception_high"] = "{:,d}".format(
        db.TicketDoc.find(
            {
                "source": "nessus",
                "false_positive": False,
    ...
    all_time_opened_and_closed["total_closed_since_inception_medium"] = "{:,d}".format(
        db.TicketDoc.find(
            {
                "source": "nessus",
                "false_positive": False,
    ...
    all_time_opened_and_closed["total_closed_since_inception_low"] = "{:,d}".format(
        db.TicketDoc.find(
            {
                "source": "nessus",
                "false_positive": False,
...
def report(db, orgs, start, end):
    ...
    closed_tix.append(
        db.tickets.find(
            {"source": "nessus", "time_closed": {"$gte": start, "$lte": end}}
        ).count()
    )
    closed_tix.append(
        db.tickets.find(
        ...
        db.reports.find(
            {
                ...
                "generated_time": {"$gte": start, "$lte": end},
            }
        ).count()
    )
    reports.append(
...
def total_open(db, orgs):
    ...
    open_non_FP_tix["total"] = "{:,}".format(
        db.tickets.find(
            {"open": True, "source": "nessus", "false_positive": False}
        ).count()
    )
    open_non_FP_tix["federal"] = "{:,}".format(
        db.tickets.find(
            {
                "open": True,
                "source": "nessus",
    ...
    open_non_FP_tix["private"] = "{:,}".format(
        db.tickets.find(
            {
                "open": True,
                "source": "nessus",
    ...
    open_FP_tix = dict()
    open_FP_tix["total"] = "{:,}".format(
        db.TicketDoc.find(
            {"open": True, "source": "nessus", "false_positive": True}
        ).count()
    )
    open_FP_tix["federal"] = "{:,}".format(
        db.tickets.find(
            {
                "open": True,
                "source": "nessus",
...
def get_stats(db):
    orgs = db.RequestDoc.get_owner_types(
        as_lists=True, stakeholders_only=False, include_retired=True
    )
    orgs["SLTT"] = (
        orgs[AGENCY_TYPE.STATE]
        + orgs[AGENCY_TYPE.LOCAL]
        + orgs[AGENCY_TYPE.TRIBAL]
        + orgs[AGENCY_TYPE.TERRITORIAL]
    )
    now = util.utcnow()
    ...
    ticket["currentFYactivity"] = return_activity(
        db, orgs, "Current FY Activity", rd["fy_start"], now
    )
    ticket["previousFYactivity"] = return_activity(
        db, orgs, "Previous FY Activity", rd["prev_fy_start"], rd["prev_fy_end"]
    )
    ticket["currentMonthActivity"] = return_activity(
        db, orgs, "Current Month Activity", rd["month_start"], now
    )
    ...
    ticket["currentWeekActivity"] = return_activity(
        db, orgs, "Current Week Activity", rd["week_start"], now
    )
    ticket["previousWeekActivity"] = return_activity(
        db, orgs, "Previous Week Activity", rd["prev_week_start"], rd["prev_week_end"]
    )
...
def main():
    ...
    print "Currently-Open Non-False-Positive Tickets:", stats["open_non_FP_tix"][
        "total"
    ]
    ...
    print_activity(
        stats["currentFYactivity"], "Current FY Activity", rd["fy_start"], rd["now"]
    )
    print_activity(
