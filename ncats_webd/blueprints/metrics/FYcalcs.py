...
def vuln_ticket_counts(FY_START, FY_END, db, severity=0):
    ...
    if severity == 0:
        opened_before_and_still_open = db.TicketDoc.find(
            {
                "source": "nessus",
                "false_positive": False,
                "time_opened": {"$lt": FY_START},
                "open": True,
            }
        ).count()
        opened_before_and_closed_during = db.TicketDoc.find(
            {
                "source": "nessus",
                "false_positive": False,
                "time_opened": {"$lt": FY_START},
                "time_closed": {"$gte": FY_START, "$lt": FY_END},
            }
        ).count()
        opened_during_and_still_open = db.TicketDoc.find(
            {
                "source": "nessus",
                "false_positive": False,
                "time_opened": {"$gte": FY_START, "$lt": FY_END},
                "open": True,
            }
        ).count()
        opened_during_and_closed_during = db.TicketDoc.find(
            {
                "source": "nessus",
                "false_positive": False,
                "time_opened": {"$gte": FY_START, "$lt": FY_END},
                "time_closed": {"$lt": FY_END},
            }
        ).count()
        opened_during_and_closed_after = db.TicketDoc.find(
            {
                "source": "nessus",
                "false_positive": False,
                "time_opened": {"$gte": FY_START, "$lt": FY_END},
                "time_closed": {"$gte": FY_END},
            }
        ).count()
        fp_before = db.TicketDoc.find(
            {
                "source": "nessus",
                "false_positive": True,
                "time_opened": {"$lt": FY_START},
        ...
        fp_after = db.TicketDoc.find(
            {
                "source": "nessus",
                "false_positive": True,
                "time_opened": {"$gte": FY_END},
            }
        ).count()

        vulnerable_host_count = len(
            db.TicketDoc.find(
                {
                    "source": "nessus",
                    "$or": [
    ...
    output["opened_before_and_still_open"] = "{:,d}".format(
        opened_before_and_still_open
    )
    output["opened_before_and_closed_during"] = "{:,d}".format(
        opened_before_and_closed_during
    )
    output["opened_during_and_still_open"] = "{:,d}".format(
        opened_during_and_still_open
    )
    output["opened_during_and_closed_during"] = "{:,d}".format(
        opened_during_and_closed_during
    )
    output["opened_during_and_closed_after"] = "{:,d}".format(
        opened_during_and_closed_after
    )
    ...
    if float(vulnerable_host_count > 0):
        output["avg_vulns_per_vulnerable_host"] = "{:.2f}".format(
            total_vulns / float(vulnerable_host_count)
        )
    else:
...
def print_vuln_ticket_counts(FY_START, FY_END, obj):
    print "\n{:}Vulnerability tickets opened before {:} and still open now: {:}".format(
        obj["severity_level"], FY_START, obj["opened_before_and_still_open"]
    )
    print "{:}Vulnerability tickets opened before {:} and closed by {:}: {:}".format(
        obj["severity_level"], FY_START, FY_END, obj["opened_before_and_closed_during"]
    )
    print "{:}Vulnerability tickets opened between {:} and {:} and still open now: {:}".format(
        obj["severity_level"], FY_START, FY_END, obj["opened_during_and_still_open"]
    )
    print "{:}Vulnerability tickets opened after {:} and closed by {:}: {:}".format(
        obj["severity_level"], FY_START, FY_END, obj["opened_during_and_closed_during"]
    )
    print "{:}Vulnerability tickets opened between {:} and {:}, and closed after {:}: {:}".format(
        obj["severity_level"],
        FY_START,
        FY_END,
        FY_END,
        obj["opened_during_and_closed_after"],
    )
    print "False positives tickets opened before {:}: {:}".format(
        FY_START, obj["fp_before"]
    )
    print "False positives tickets opened between {:} and {:}: {:}".format(
        FY_START, FY_END, obj["fp_during"]
    )
    print "False positives tickets opened after {:}: {:}".format(
        FY_END, obj["fp_after"]
    )
    print "-----------------------------------------------------"
    print "Total {:}vulnerability tickets open at any point between {:} and {:}: {:}".format(
        obj["severity_level"], FY_START, FY_END, obj["total_vulns"]
    )
    print "Total vulnerable hosts for tickets above: {:}".format(
        obj["vulnerable_host_count"]
    )
    print "Average vulnerabilities per vulnerable host: {:}".format(
        obj["avg_vulns_per_vulnerable_host"]
    )
...
def top_vulns(FY_START, FY_END, db, severity=0):
    ...
    if severity == 0:
        pipeline = [
            ...
            {
                "$group": {
                    "_id": {"source_id": "$source_id", "details_name": "$details.name"},
                    "count": {"$sum": 1},
                }
            },
            {"$sort": {"count": -1}},
...
def top_OS(FY_START, FY_END, db):
    ...
    result = list(
        db.HostScanDoc.collection.aggregate(pipeline, allowDiskUse=True, cursor={})
    )
    total = 0
    for j in range(0, len(result)):
        total = total + int(result[j]["count"])
...
def top_services(FY_START, FY_END, db):
    ...
    result = list(
        db.PortScanDoc.collection.aggregate(pipeline, allowDiskUse=True, cursor={})
    )
    total = 0
    for j in range(0, len(result)):
        total = total + int(result[j]["count"])
...
def num_orgs_scanned(FY_START, FY_END, db):
    total_orgs = db.SnapshotDoc.find(
        {
            "last_change": {"$gte": FY_START, "$lt": FY_END},
            "owner": {"$in": ALL_STAKEHOLDERS_BY_TYPE["ALL"]},
        }
    ).distinct("owner")
    fed_orgs = db.SnapshotDoc.find(
        {
            "last_change": {"$gte": FY_START, "$lt": FY_END},
            "owner": {"$in": ALL_STAKEHOLDERS_BY_TYPE[AGENCY_TYPE.FEDERAL]},
        }
    ).distinct("owner")
    sltt_orgs = db.SnapshotDoc.find(
        {
            "last_change": {"$gte": FY_START, "$lt": FY_END},
            "owner": {"$in": ALL_STAKEHOLDERS_BY_TYPE["SLTT"]},
        }
    ).distinct("owner")
    private_orgs = db.SnapshotDoc.find(
        {
            "last_change": {"$gte": FY_START, "$lt": FY_END},
            "owner": {"$in": ALL_STAKEHOLDERS_BY_TYPE[AGENCY_TYPE.PRIVATE]},
        }
    ).distinct("owner")
    ...
    if len(total_orgs) > 0:
        output["fed_orgs_pct"] = round(
            (float(len(fed_orgs)) / len(total_orgs)) * 100, 1
        )
        output["sltt_orgs_pct"] = round(
            (float(len(sltt_orgs)) / len(total_orgs)) * 100, 1
        )
        output["private_orgs_pct"] = round(
            (float(len(private_orgs)) / len(total_orgs)) * 100, 1
        )
    else:
...
def print_num_orgs_scanned(FY_START, FY_END, obj):
    ...
    print "* FEDERAL stakeholder organizations: {:} ({:}%)".format(
        obj["fed_orgs"], obj["fed_orgs_pct"]
    )
    print "* SLTT stakeholder organizations: {:} ({:}%)".format(
        obj["sltt_orgs"], obj["sltt_orgs_pct"]
    )
    print "* PRIVATE stakeholder organizations: {:} ({:}%)".format(
        obj["private_orgs"], obj["private_orgs_pct"]
    )
    print obj["description"]


def num_addresses_hosts_scanned(FY_START, FY_END, db):
    ORGS_SCANNED_DURING_FY = db.SnapshotDoc.find(
        {"last_change": {"$gte": FY_START, "$lt": FY_END}}
    ).distinct("owner")

    total_uniq_ips = db.HostDoc.find(
        {
            "owner": {"$in": ORGS_SCANNED_DURING_FY},
            "last_change": {"$gte": FY_START, "$lt": (FY_END + datetime.timedelta(30))},
            "state.reason": {"$ne": "new"},
        }
    ).count()
    fed_uniq_ips = db.HostDoc.find(
        {
            "owner": {
                "$in": list(
                    set(ALL_ORGS_BY_TYPE[AGENCY_TYPE.FEDERAL])
                    & set(ORGS_SCANNED_DURING_FY)
                )
            },
            "last_change": {"$gte": FY_START, "$lt": (FY_END + datetime.timedelta(30))},
            "state.reason": {"$ne": "new"},
    ...
    sltt_uniq_ips = db.HostDoc.find(
        {
            "owner": {
                "$in": list(set(ALL_ORGS_BY_TYPE["SLTT"]) & set(ORGS_SCANNED_DURING_FY))
            },
            "last_change": {"$gte": FY_START, "$lt": (FY_END + datetime.timedelta(30))},
            "state.reason": {"$ne": "new"},
        }
    ).count()
    private_uniq_ips = db.HostDoc.find(
        {
            "owner": {
                "$in": list(
                    set(ALL_ORGS_BY_TYPE[AGENCY_TYPE.PRIVATE])
                    & set(ORGS_SCANNED_DURING_FY)
                )
            },
            "last_change": {"$gte": FY_START, "$lt": (FY_END + datetime.timedelta(30))},
    ...
    if total_uniq_ips > 0:
        ...
        output["sltt_addr_pct"] = round(
            (float(sltt_uniq_ips) / total_uniq_ips) * 100, 1
        )
        output["private_addr_pct"] = round(
            (float(private_uniq_ips) / total_uniq_ips) * 100, 1
        )
...
---"""
    ...
    for (org_group, host_group) in [
        (ALL_ORGS_BY_TYPE[AGENCY_TYPE.FEDERAL], "fed_hosts_scanned"),
        (ALL_ORGS_BY_TYPE["SLTT"], "sltt_hosts_scanned"),
        (ALL_ORGS_BY_TYPE[AGENCY_TYPE.PRIVATE], "private_hosts_scanned"),
    ]:
        pipeline = [
            {
                "$match": {
                    "owner": {"$in": org_group},
                    "time": {"$gte": FY_START, "$lt": FY_END},
                }
            },
    ...
    if total_hosts_scanned > 0:
        output["fed_hosts_scanned_pct"] = round(
            (float((result["fed_hosts_scanned"])) / (total_hosts_scanned)) * 100, 1
        )
        output["sltt_hosts_scanned_pct"] = round(
            (float((result["sltt_hosts_scanned"])) / (total_hosts_scanned)) * 100, 1
        )
        output["private_hosts_scanned_pct"] = round(
            (float((result["private_hosts_scanned"])) / (total_hosts_scanned)) * 100, 1
        )
...
---"""
    ...
    total_vuln_hosts = db.TicketDoc.find(
        {
            "source": "nessus",
            "false_positive": False,
            "time_opened": {"$gte": FY_START, "$lt": FY_END},
    ...
    fed_vuln_hosts = db.TicketDoc.find(
        {
            "source": "nessus",
            "false_positive": False,
            "time_opened": {"$gte": FY_START, "$lt": FY_END},
            "owner": {"$in": ALL_ORGS_BY_TYPE[AGENCY_TYPE.FEDERAL]},
        }
    ).distinct("ip_int")
    sltt_vuln_hosts = db.TicketDoc.find(
        {
            "source": "nessus",
            "false_positive": False,
            "time_opened": {"$gte": FY_START, "$lt": FY_END},
            "owner": {"$in": ALL_ORGS_BY_TYPE["SLTT"]},
        }
    ).distinct("ip_int")
    private_vuln_hosts = db.TicketDoc.find(
        {
            "source": "nessus",
            "false_positive": False,
            "time_opened": {"$gte": FY_START, "$lt": FY_END},
            "owner": {"$in": ALL_ORGS_BY_TYPE[AGENCY_TYPE.PRIVATE]},
        }
    ).distinct("ip_int")

    if total_vuln_hosts > 0:
        output["fed_vuln_hosts_pct"] = round(
            (float(len(fed_vuln_hosts)) / len(total_vuln_hosts)) * 100, 1
        )
        output["sltt_vuln_hosts_pct"] = round(
            (float(len(sltt_vuln_hosts)) / len(total_vuln_hosts)) * 100, 1
        )
        output["private_vuln_hosts_pct"] = round(
            (float(len(private_vuln_hosts)) / len(total_vuln_hosts)) * 100, 1
        )
...
def print_num_addresses_hosts_scanned(FY_START, FY_END, obj):
    ...
    print "* PRIVATE addresses: {:} ({:}%)".format(
        obj["private_addr"], obj["private_addr_pct"]
    )
    print obj["description1"]

    print "\nTotal hosts scanned: {:}".format(obj["total_hosts_scanned"])
    print "* FEDERAL hosts: {:} ({:}%)".format(
        obj["fed_hosts_scanned"], obj["fed_hosts_scanned_pct"]
    )
    print "* SLTT hosts: {:} ({:}%)".format(
        obj["sltt_hosts_scanned"], obj["sltt_hosts_scanned_pct"]
    )
    print "* PRIVATE hosts: {:} ({:}%)".format(
        obj["private_hosts_scanned"], obj["private_hosts_scanned_pct"]
    )
    ...
    print "\nTotal vulnerable hosts detected during scans: {:}".format(
        obj["total_vuln_hosts"]
    )
    print "* FEDERAL vulnerable hosts: {:} ({:}%)".format(
        obj["fed_vuln_hosts"], obj["fed_vuln_hosts_pct"]
    )
    print "* SLTT vulnerable hosts: {:} ({:}%)".format(
        obj["sltt_vuln_hosts"], obj["sltt_vuln_hosts_pct"]
    )
    print "* PRIVATE vulnerable hosts: {:} ({:}%)".format(
        obj["private_vuln_hosts"], obj["private_vuln_hosts_pct"]
    )
    print obj["description4"]


def num_reports_generated(FY_START, FY_END, db):
    total_reports = db.ReportDoc.find(
        {
            "generated_time": {"$gte": FY_START, "$lt": FY_END},
            "report_types": REPORT_TYPE.CYHY,
        }
    )
    fed_reports = db.ReportDoc.find(
        {
            "generated_time": {"$gte": FY_START, "$lt": FY_END},
            "report_types": REPORT_TYPE.CYHY,
            "owner": {"$in": ALL_ORGS_BY_TYPE[AGENCY_TYPE.FEDERAL]},
    ...
    sltt_reports = db.ReportDoc.find(
        {
            "generated_time": {"$gte": FY_START, "$lt": FY_END},
            "report_types": REPORT_TYPE.CYHY,
            "owner": {"$in": ALL_ORGS_BY_TYPE["SLTT"]},
    ...
    private_reports = db.ReportDoc.find(
        {
            "generated_time": {"$gte": FY_START, "$lt": FY_END},
            "report_types": REPORT_TYPE.CYHY,
            "owner": {"$in": ALL_ORGS_BY_TYPE[AGENCY_TYPE.PRIVATE]},
    ...
    output["fed_reports_pct"] = round(
        (float(fed_reports.count()) / total_reports.count()) * 100, 1
    )
    output["sltt_reports"] = "{:,d}".format(sltt_reports.count())
    output["sltt_reports_pct"] = round(
        (float(sltt_reports.count()) / total_reports.count()) * 100, 1
    )
    output["private_reports"] = "{:,d}".format(private_reports.count())
    output["private_reports_pct"] = round(
        (float(private_reports.count()) / total_reports.count()) * 100, 1
    )
    output[
        "description"
    ] = """---
...
def print_num_reports_generated(FY_START, FY_END, obj):
    ...
    print "* FEDERAL CyHy reports: {:} ({:}%)".format(
        obj["fed_reports"], obj["fed_reports_pct"]
    )
    print "* SLTT CyHy reports: {:} ({:}%)".format(
        obj["sltt_reports"], obj["sltt_reports_pct"]
    )
    print "* PRIVATE CyHy reports: {:} ({:}%)".format(
        obj["private_reports"], obj["private_reports_pct"]
    )
    print obj["description"]
...
def avg_cvss_score(FY_START, FY_END, db):
    pipeline = [
        ...
        {
            "$group": {
                "_id": {"ip_int": "$ip_int", "snapshots": "$snapshots",},
                "cvss_max": {"$max": "$cvss_base_score"},
            }
        },  # host cvss is the max of any cvss for that host
        {"$group": {"_id": {}, "cvss_avg": {"$avg": "$cvss_max"},}},
    ]  # get avg of all host maximums
...
def print_avg_cvss_score(FY_START, FY_END, obj):
    print "\nAverage CVSS score for hosts with vulnerabilities: {:.3}".format(
        obj["overall_cvss"]
    )
...
def print_unique_vulns(FY_START, FY_END, obj):
    for item in obj["data"]:
        print "\n{:} unique vulnerabilities discovered: {:}".format(
...
def print_new_vuln_detections(FY_START, FY_END, obj):
    for item in obj["data"]:
        print "\n{:} vulnerability tickets opened: {:}".format(
            item["type"], item["total"]
        )
        print "  * Critical: {:} ({:}%)".format(item["crit"], item["crit_pct"])
        print "  * High: {:} ({:}%)".format(item["high"], item["high_pct"])
...
def categorize_orgs(db):
    ...
    ALL_ORGS_BY_TYPE = db.RequestDoc.get_owner_types(
        as_lists=True, stakeholders_only=False
    )
    ALL_ORGS_BY_TYPE["SLTT"] = (
        ALL_ORGS_BY_TYPE[AGENCY_TYPE.STATE]
        + ALL_ORGS_BY_TYPE[AGENCY_TYPE.LOCAL]
        + ALL_ORGS_BY_TYPE[AGENCY_TYPE.TRIBAL]
        + ALL_ORGS_BY_TYPE[AGENCY_TYPE.TERRITORIAL]
    )

    ALL_STAKEHOLDERS_BY_TYPE = db.RequestDoc.get_owner_types(
    ...
    ALL_STAKEHOLDERS_BY_TYPE["SLTT"] = (
        ALL_STAKEHOLDERS_BY_TYPE[AGENCY_TYPE.STATE]
        + ALL_STAKEHOLDERS_BY_TYPE[AGENCY_TYPE.LOCAL]
        + ALL_STAKEHOLDERS_BY_TYPE[AGENCY_TYPE.TRIBAL]
        + ALL_STAKEHOLDERS_BY_TYPE[AGENCY_TYPE.TERRITORIAL]
    )
