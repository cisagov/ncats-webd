...
def fema_detail(db):
    ...
    i_num = ii_num = iii_num = iv_num = v_num = vi_num = vii_num = viii_num = ix_num = x_num = 0
    ...
    for customer in customers:
        ...
        try:
            ...
            state = 'N/A'
        try:
            reports = db.reports.find({'owner': customer['_id']}).sort([('generated_time',1)]).limit(1)
    ...
    fema_list.insert(0,(i_num,ii_num,iii_num,iv_num,v_num,vi_num,vii_num,viii_num,ix_num,x_num))
    return fema_list
...
def fema_csv(db):
    ...
    for customer in customers:
        ...
        output.write(str(region) + ',' + name + ',' + sector + ',' + city + ',' + state + ',' + str(first_scan) + '\n')
