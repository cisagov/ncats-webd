...
def congressional_data(db,start_date,end_date):
    ...
    all_stakeholders = db.RequestDoc.get_owner_types(as_lists=True, stakeholders_only=True)

    active_fed_stakeholders = db.ReportDoc.find({'owner':{'$in':all_stakeholders[AGENCY_TYPE.FEDERAL]}, 'generated_time':{'$gte':start_date, '$lt':end_date}}).distinct('owner')
    ...
    active_fed_executive_stakeholders = db.ReportDoc.find({'owner':{'$in':fed_executive_stakeholders}, 'generated_time':{'$gte':start_date, '$lt':end_date}}).distinct('owner')
    ...
    active_fed_cfo_stakeholders = db.ReportDoc.find({'owner':{'$in':fed_cfo_stakeholders}, 'generated_time':{'$gte':start_date, '$lt':end_date}}).distinct('owner')
    ...
    fed_exec_non_cfo_stakeholders = list(set(fed_executive_stakeholders) - set(fed_cfo_stakeholders))
    active_fed_exec_non_cfo_stakeholders = db.ReportDoc.find({'owner':{'$in':fed_exec_non_cfo_stakeholders}, 'generated_time':{'$gte':start_date, '$lt':end_date}}).distinct('owner')
    ...
    SLTT_stakeholders = all_stakeholders[AGENCY_TYPE.STATE] + all_stakeholders[AGENCY_TYPE.LOCAL] + all_stakeholders[AGENCY_TYPE.TRIBAL] + all_stakeholders[AGENCY_TYPE.TERRITORIAL]
    active_SLTT_stakeholders = db.ReportDoc.find({'owner':{'$in':SLTT_stakeholders}, 'generated_time':{'$gte':start_date, '$lt':end_date}}).distinct('owner')
    ...
    active_private_stakeholders = db.ReportDoc.find({'owner':{'$in':all_stakeholders[AGENCY_TYPE.PRIVATE]}, 'generated_time':{'$gte':start_date, '$lt':end_date}}).distinct('owner')
    ...
    mylist = {}
    print 'Date Range: {} - {}'.format(start_date.strftime('%Y-%m-%d %H:%M UTC'), end_date.strftime('%Y-%m-%d %H:%M UTC'))
    ...
    for (stakeholder_list, group_name) in ((active_fed_owners, 'FEDERAL'), (active_fed_executive_owners, 'FEDERAL EXECUTIVE'), (active_fed_cfo_owners, 'FEDERAL EXECUTIVE - CFO ACT'), (active_fed_exec_non_cfo_owners, 'FEDERAL EXECUTIVE - NON-CFO ACT')):
        ...
        if isnull(median_days_to_mitigate_criticals):
            ...
            myarg = {group_name.replace(" ", "_").replace("-","_") + '_median_time_to_mitigate':'No Critical Vulnerabilities Mitigated'}
        else:
            print '  Median time to mitigate Critical vulnerabilities: {0:,g} days'.format(median_days_to_mitigate_criticals)
            myarg = {group_name.replace(" ", "_").replace("-","_") + '_median_time_to_mitigate':'{0:,g}'.format(median_days_to_mitigate_criticals)}
        mylist.update(myarg)

        (pipeline, collection) = opened_in_date_range_open_ticket_age_pl(stakeholder_list, start_date, end_date)
        output = db[collection].aggregate(pipeline, cursor={})
        df = DataFrame(list(output))
        median_days_active_criticals = round(df.loc[df['severity'] == 4]['open_ticket_duration'].median() / (24*60*60*1000.0))
        if isnull(median_days_active_criticals):
            ...
            myarg = {group_name.replace(" ", "_").replace("-","_") + '_currently_active':'No Critical Vulnerabilities Currently Active'}
        else:
            print '  Median time Critical vulnerabilities currently active: {0:,g} days'.format(median_days_active_criticals)
            myarg = {group_name.replace(" ", "_").replace("-","_") + '_currently_active':'{0:,g}'.format(median_days_active_criticals)}
        mylist.update(myarg)
        print
    ...
    for (stakeholder_list, group_name) in ((active_fed_stakeholders, 'FEDERAL'), (active_fed_executive_stakeholders, 'FEDERAL EXECUTIVE'), (active_fed_cfo_stakeholders, 'FEDERAL EXECUTIVE - CFO ACT'), (active_fed_exec_non_cfo_stakeholders, 'FEDERAL EXECUTIVE - NON-CFO ACT'), (active_SLTT_stakeholders, 'SLTT'), (active_private_stakeholders, 'PRIVATE')):
        print '{}: {} stakeholders'.format(group_name, len(stakeholder_list))
        myarg = {group_name.replace(" ", "_").replace("-","_") + '_active_stakeholders':'{0:,g}'.format(len(stakeholder_list))}
        mylist.update(myarg)
    ...
    # Use "_stakeholders" lists here because we DO NOT want to include descendent orgs in these metrics
    for (stakeholder_list, group_name) in ((active_fed_stakeholders, 'FEDERAL'), (active_fed_executive_stakeholders, 'FEDERAL EXECUTIVE'), (active_fed_cfo_stakeholders, 'FEDERAL EXECUTIVE - CFO ACT'), (active_fed_exec_non_cfo_stakeholders, 'FEDERAL EXECUTIVE - NON-CFO ACT')):
        myarg = {group_name.replace(" ", "_").replace("-","_") + '_Active_CyHy_Stakeholders':stakeholder_list}
        mylist.update(myarg)
