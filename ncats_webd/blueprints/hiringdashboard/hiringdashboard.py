...
def start_background_thread():
    ...
    if new_hire_feed is None:
        new_hire_feed = NewHireFeed(current_app.new_hire_db, broadcast_new_hire_metrics(), current_app.logger)
    ...
    try:
        for doc in metrics:
            ...
            for stage in range(0, int(doc['current_stage'])-1):
                ...
                weeks_in_stage[stage] = round((doc['date_stage_entered'][stage+1] - doc['date_stage_entered'][stage]).days / 7.0)
            # Difference in current stage and insert date
            weeks_in_stage[stage+1] = round((doc['insert_date'] - doc['date_stage_entered'][stage+1]).days / 7.0)
            # Append the number of weeks in stage to each Doc
            doc['weeks_in_stage'] = weeks_in_stage
    except Exception as e:
        print('Trying to do arithmetic on null. This shouldn\'t happen unless someone manually changed the database.'
              ' \n Error: {}'.format(e))
...
def broadcast_new_hire_metrics():
    ...
    socketio.emit('new_hire_metrics', {'data':new_hire_metrics}, namespace='/cyhy', room='new_hire_metrics')
